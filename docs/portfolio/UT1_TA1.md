...
La actividad consiste en realizar un análisis de los datos del dataset del Titanic. El objetivo es investigar los factores que influyeron en la supervivencia de los pasajeros, explorando variables como sexo, edad, clase del boleto, familiares a bordo y tarifa pagada.

Utilizando herramientas como Pandas, Seaborn y Matplotlib para la carga, limpieza y visualización de datos.

Se busca desarrollar habilidades de análisis de datos y visualización aplicadas a un caso.

## Objetivos

- Comprender la estructura y atributos del dataset.
- Identificar la variable objetivo (Survived).
- Detectar y analizar valores faltantes.
- Explorar visualmente relaciones entre variables y la supervivencia.
- Reflexionar sobre qué factores pudieron haber sido más determinantes en las probabilidades de sobrevivir.
...
Estos son los tipicos que cualquiera podria imaginar:
...
- Edad: los niños siempre primero.
- Tarifa: esta relacionada con clase, por lo tanto, con probabilidad de supervivencia.

#### ¿Qué desafíos de calidad de datos esperas encontrar?

Seguramente alguno de:

- Valores faltantes en en alguna variable.
- Datos no estructurados en alguna variable que requieran limpieza o transformación.
- Distribución sesgada en algunas variables, por ejemplo muchos casos altos o muchos bajos.

#### ¿Qué variables podrían estar correlacionadas?

- Pclass y Fare: pasajes más caros corresponden a clases más altas.
- SibSp y Parch: el tema de familias.
- Sex y Survived: mujeres más propensas a sobrevivir, aunque no es tan directa, es mas por un hecho cultural.
- Age y Survived: niños más propensos a sobrevivir, mismo que sex.
...
Las variables que parecen más relacionadas con Survived son:
...
Hay más valores faltantes en:

- Cabin: demasiados valores nulos, probablemente convenga descartar.
- Age: imputación con mediana.
- Embarked: imputar con la moda.

Quizas sea bueno probar si:

- Los pasajeros de 2ª clase tuvieron mayor probabilidad de supervivencia que los de 3ª.
- Las mujeres tuvieron más chances de sobrevivir que los hombres, independientemente de la clase.

Como conclusión, logré comprender el flujo básico de un EDA, incluyendo la carga del dataset, la exploración inicial, la visualización y el análisis de las variables.

A partir del análisis, pude observar que el sexo y la clase social fueron factores determinantes en la supervivencia de los pasajeros del Titanic.

Por otra parte, identifiqué que existen valores faltantes en algunas columnas, lo que requiere aplicar estrategias adecuadas, ya sea de imputación de datos o descartar variables/filas.
